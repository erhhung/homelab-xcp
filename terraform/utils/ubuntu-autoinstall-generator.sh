...
# https://github.com/KolinSmith/ubuntu-24.04-autoinstall-generator
...
Usage: $(basename "${BASH_SOURCE[0]}") [-h] [-v] [-a] [-e] [-u user-data-file] [-m meta-data-file]
                        [-k] [-c] [-r] [-s source-iso-file] [-d destination-iso-file]
...
-a, --all-in-one        Bake user-data and meta-data into the generated ISO. By default you will need to
                        boot systems with a CIDATA volume attached containing your autoinstall user-data
                        and meta-data files.
                        For more information see: https://ubuntu.com/server/docs/install/autoinstall-quickstart
-e, --use-hwe-kernel    Force the generated ISO to boot using the hardware enablement (HWE) kernel.
                        Not supported by early Ubuntu ${UBUNTU_VERSION} release ISOs.
-u, --user-data         Path to user-data file. Required if using -a
-m, --meta-data         Path to meta-data file. Will be an empty file if not specified and using -a
-k, --no-verify         Disable GPG verification of the source ISO file. By default SHA256SUMS-$today
                        and SHA256SUMS-$today.gpg in ${script_dir} will be used to verify the authenticity
                        and integrity of the source ISO file. If they are not present the latest daily
                        SHA256SUMS will be downloaded and saved in ${script_dir}. The Ubuntu signing
                        key will be downloaded and saved in a new keyring in ${script_dir}
-c, --no-md5            Disable MD5 checksum on boot
-r, --use-release-iso   Use the current release ISO instead of the daily ISO. The file will be used if it
                        already exists.
-s, --source            Source ISO file. By default the latest daily ISO for Ubuntu ${UBUNTU_VERSION} will
                        be downloaded and saved as ${script_dir}/ubuntu-original-$today.iso
                        That file will be used by default if it already exists.
-d, --destination       Destination ISO file. By default ${script_dir}/ubuntu-autoinstall-$today.iso will
                        be created, overwriting any existing file.
EOF
  exit
}
...
if [ ! -f "${source_iso}" ]; then
  ...
  if [ ${gpg_verify} -eq 1 ]; then
    if [ "${source_iso}" != "${script_dir}/${original_iso}" ]; then
      log "⚠️ Automatic GPG verification is enabled. If the source ISO file"
      log "is not the latest daily or release image, verification will fail!"
    fi
  fi
fi
...
if [ ${gpg_verify} -eq 1 ]; then
  ...
  if [ ! -f "${script_dir}/${ubuntu_gpg_key_id}.keyring" ]; then
    ...
    gpg -q --no-default-keyring --keyring "${script_dir}/${ubuntu_gpg_key_id}.keyring" \
      --keyserver "hkps://keyserver.ubuntu.com" --recv-keys "${ubuntu_gpg_key_id}"
  ...
  gpg -q --keyring "${script_dir}/${ubuntu_gpg_key_id}.keyring" \
    --verify "${script_dir}/SHA256SUMS-${sha_suffix}.gpg" \
    "${script_dir}/SHA256SUMS-${sha_suffix}" 2>/dev/null
  if [ $? -ne 0 ]; then
...
pushd "$tmpdir/iso"
xorriso -as mkisofs -r -V "ubuntu-autoinstall-$today" -o "${destination_iso}" \
